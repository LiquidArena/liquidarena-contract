...
contract LPBattleVault is IERC721Receiver {
    ...
    function resolveBattle(uint256 battleId) external {
        ...
        INonfungiblePositionManager.CollectParams memory paramsCreator = INonfungiblePositionManager.CollectParams({
            tokenId: b.creatorTokenId,
            recipient: winner,
            amount0Max: type(uint128).max,
            amount1Max: type(uint128).max
        });
        INonfungiblePositionManager.CollectParams memory paramsOpponent = INonfungiblePositionManager.CollectParams({
